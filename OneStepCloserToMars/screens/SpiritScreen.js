...
const BASE_URL = 'https://api.nasa.gov/mars-photos/api/v1/rovers/spirit/photos';
const SPIRIT_SOL_OFFSET = 1;

const SpiritScreen = () => {
    const [photos, setPhotos] = useState([]);
    const [isLoading, setIsLoading] = useState(false);
    const [sol, setSol] = useState(SPIRIT_SOL_OFFSET);
    ...
    const handleApiCall = useCallback(async () => {
        if (!isLoading) {
            ...
            fetch(`${BASE_URL}?camera=PANCAM&sol=${sol}&api_key=${API_KEY}`)
                ...
                .then((json) => {
                    console.debug(`Spirit, sol: ${sol}, photos: ${json.photos.length}`);
                    setSol(sol + 1)
